...
/** How long one member failure counts toward the same stuck signal. */
const FAILURE_WINDOW_MS = 600_000
/** Failures inside one window before the chair is woken about a stuck member. */
...
/**
 * Wrap one message body so the true sender stays clear. A member often writes
 * its own `[name]` tag already, because the room rules ask it to identify
 * itself. The wrapper is skipped in that case, so the receiver never sees the
 * tag twice.
 */
function wrap(senderName, text) {
  ...
  if (head.startsWith(tag)) return head
  return `${tag} ${body}`
}
...
function withRoomLock(roomId, task) {
  ...
  /* Release the entry once the chain drains, so the map holds only rooms with a
   * task still in flight. A later task replaces the entry before this runs, and
   * the identity check keeps that entry. */
  tail.then(() => {
...
export function apply(ctx, config) {
  ...
  /** Failure count per room and member, for the stuck-member notice. */
  const memberFailures = new Map()
  ...
  /**
   * Record one failed member request. The notice always reaches the chair
   * mailbox. The third failure inside ten minutes also wakes the chair, through
   * a live member, the way every other notice travels.
   */
  async function noteMemberFailure(roomId, memberName, failure) {
    const key = `${roomId}\u0000${memberName}`
    const now = Date.now()
    const seen = memberFailures.get(key)
    const count = seen === undefined || now - seen.at > FAILURE_WINDOW_MS ? 1 : seen.count + 1
    memberFailures.set(key, { count, at: now })
    /* The map is a rolling cache, so it never grows without bound. */
    if (memberFailures.size > 200) {
      for (const [other, row] of memberFailures) {
        if (now - row.at > FAILURE_WINDOW_MS) memberFailures.delete(other)
      }
    }
    ...
    const base = `${memberName}: model request failed (${failure})`
    const text =
      count >= STUCK_FAILURES
        ? `${base}. Failure ${count} in ten minutes. Nudge the member once, then replace it.`
        : base
    ...
    trace(`member failure noted for ${memberName}: ${failure}`)

    if (count < STUCK_FAILURES) return
    const sender = liveSenderFor(room, memberName)
    if (sender === undefined) return
    await wakeFor(room, sender, CHAIR, text).catch((error) => {
      log(`chair wake for a stuck member failed: ${error?.message ?? error}`)
    })
  }

  /** One live member name to send through, preferring the member in question. */
  function liveSenderFor(room, preferred) {
    const names = [preferred, ...(room.members ?? []).map((member) => member.name)]
    for (const name of names) {
      const id = activeAgentId(room, name)
      if (id !== undefined && ctx.agents.get(String(id)) !== undefined) return name
    }
    return undefined
  }
  ...
  async function runProgress(roomId, dir) {
    ...
    for (const member of room.members ?? []) {
      ...
      lines.push([member, progressLine(agent, member)])
    }
    if (lines.length === 0) return

    /* One locked pass. The totals refresh inside the same lock, so the recorded
     * count never lags behind the mailboxes after a progress entry. */
    await withRoomLock(roomId, async () => {
      for (const [member, line] of lines) {
        try {
          await appendInbox(dir, CHAIR, {
            id: `m-${randomUUID().slice(0, 8)}`,
            from: member.name,
  ...
  function armChair(agent) {
    ...
    registrations.set(agentId, owned)
    /* Arming is per session, so the line is verbose only. A count would
     * confuse a reader who has fewer sessions open than the harness created. */
    trace(`chair capability armed for session ${agentId}`)
  }
  ...
  /**
   * Tell the chair when a member's model request fails. Without this the chair
   * sees only silence and cannot apply the rule in the spec: nudge a silent
   * member once, then replace it. One failure is written to the chair mailbox.
   * The third failure inside ten minutes also wakes the chair, because that is
   * a stuck member rather than a blip.
   */
  ctx.on('agent/request-error', async (payload, next) => {
    try {
      const indexed = memberIndex.get(String(payload?.agent?.id ?? ''))
      if (indexed !== undefined) {
        const failure = payload?.failure ?? {}
        const text =
          [failure.status, failure.code, failure.message].filter(Boolean).join(' ') ||
          'unknown failure'
        noteMemberFailure(indexed.roomId, indexed.member, text).catch((error) => {
          log(`member failure notice failed: ${error?.message ?? error}`)
        })
      }
    } catch (error) {
      log(`member failure observer failed: ${error?.message ?? error}`)
    }
    /* This event is a waterfall. Delegate, so the platform keeps its recovery. */
    return next()
  })
  ...
  /**
   * Arm every root session that is already live. The FIRST request of a session
   * must already carry the room tools, so arming happens here and at creation.
   * Arming later does not work: the request is assembled before the message that
   * would trigger it. Idle restored sessions are armed too, and that cost is
   * accepted, because the alternative breaks the plain-language path.
   */
