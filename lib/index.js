...
export function apply(ctx, config) {
  ...
  /**
   * Arm one session when its first message arrives, not when the harness creates
   * it. A session restored at boot stays unarmed until somebody uses it, so a
   * profile with a hundred restored sessions pays for none of them.
   *
   * The message is appended before the turn's request is assembled, so the room
   * tools and the chair section are already present in that first request.
   */
  ctx.on('session/event', (session, event) => {
    ...
    try {
      ...
      log(`armChair failed for ${session.id}: ${error?.message ?? error}`)
    }
  })
