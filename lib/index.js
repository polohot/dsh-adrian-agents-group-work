...
export function apply(ctx, config) {
  ...
  /**
   * Sessions armed in this process. The per-session line is verbose only. One
   * summary line prints after the boot burst, so a busy profile stays readable.
   */
  let armedCount = 0
  let armedSummarised = false
  let armedTimer

  /** Count one armed session, and print the boot summary once. */
  function noteArmed(agentId) {
    armedCount += 1
    trace(`chair capability armed for session ${agentId}`)
    if (armedSummarised || armedTimer !== undefined) return
    armedTimer = setTimeout(() => {
      armedTimer = undefined
      armedSummarised = true
      log(`armed ${armedCount} session(s)`)
    }, 3000)
    /* The summary must never hold the process open. */
    armedTimer.unref?.()
  }
