...
export function apply(ctx, config) {
  ...
  async function deliver({ roomId, dir, room, from, to, type, text, requestId }) {
    ...
    log(`mailbox write ok: ${from} -> ${to} (${type}) held=${paused}`)
      ...
      modelTool({
        ...
        run: async (args, exec) => {
          ...
          return withRoomLock(roomId, async () => {
            ...
            log(`task created ${task.id} in ${dir}`)
            return task
          })
        },
      }),
      ...
      modelTool({
        ...
        run: async (args, exec) => {
          ...
          return withRoomLock(roomId, async () => {
            ...
            log(`task updated ${task.id} -> ${task.status} in ${dir}`)
            return task
          })
        },
      }),
  ...
  async function answerPlanRequest(dir, room, from, parsed) {
    ...
    log(`auto-approved plan request ${requestId} from ${from}`)
  }
  ...
  async function flushPending(dir, room) {
    ...
    for (const memberName of await listMailboxNames(dir)) {
      ...
      try {
        ...
        log(`delivered ${waiting.length} waiting message(s) to ${memberName}`)
      } catch (error) {
   ...
   * one short line. The line comes from a forked turn over that member's own
   * transcript, with tools denied.
   */
  function startProgress(roomId, dir) {
    ...
    log(`progress lines on for ${roomId}, every ${settings.progressIntervalMs} ms`)
  }
  ...
  async function runProgress(roomId, dir) {
    ...
    for (const member of room.members ?? []) {
      ...
      try {
        const line = await progressLine(agent, member)
  ...
  /** Run one forked summarizer turn with every tool denied. */
  async function progressLine(memberAgent, member) {
    const run = await ctx.subagents.start('fork', {
      label: `progress:${member.name}`,
      prompt: content(
        'Write one line of 3 to 5 words in the present tense that says what you are doing now. Do not call a tool. Do not explain. Reply with that line only.',
      ),
      parent: memberAgent,
      toolFilter: { allow: [] },
  ...
  async function openRoom(exec, args) {
    ...
    log(`registered ${MEMBER_TOOLS.length + CHAIR_ONLY_TOOLS.length} room tools in scope of ${chairSessionId}`)
    ...
    if (failures.length === 0) {
      ...
      const released = await flushPending(dir, room)
      log(`room ${roomId} open. ${released} waiting message(s) delivered.`)
  ...
  async function wireMember(agent) {
    ...
    log(`member tools wired for ${found.member} of room ${found.roomId}`)
  }
  ...
  async function restoreOpenRooms() {
    ...
    for (const roomId of await listDir(roomsRoot())) {
      ...
      restored += 1
      log(`restored open room ${roomId} from ${dir}`)
    }
    if (restored > 0) log(`restored ${restored} open room(s)`)
  }
